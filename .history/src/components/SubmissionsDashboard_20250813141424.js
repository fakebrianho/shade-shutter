...
export default function SubmissionsDashboard() {
	...
	const validUsers = users.filter(Boolean).filter((user, index, arr) => arr.indexOf(user) === index)
				...
				const userIdentifiers = data.submissions
					.map((s) => {
						...
						if (!s.userIdentifier && s.userInfo) {
							const fallbackId = s.userInfo.email || s.userInfo.name
							if (fallbackId) {
								console.log(`Using fallback user identifier: ${fallbackId}`)
								return fallbackId
							}
						}
					...
					>
						...
						{hasUsers ? (
							...
							<option value='' disabled>
								{hasSubmissions ? 'No valid user identifiers found' : 'No submissions yet'}
							</option>
						)}
					</select>
					<button
				...
				{submissions.length === 0 ? (
					<div className={styles.emptyState}>
						{loading ? 'Loading submissions...' : 
						 selectedUser ? `No submissions found for user: ${selectedUser}` : 
