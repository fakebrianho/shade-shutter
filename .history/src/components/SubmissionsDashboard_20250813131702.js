...
export default function SubmissionsDashboard() {
	...
	const fetchSubmissions = async () => {
		try {
			...
			if (data.success) {
				...
				const uniqueUsers = [...new Set(data.submissions.map(s => s.userIdentifier))]
	...
	const deleteSubmission = async (submissionId) => {
		if (!confirm('Are you sure you want to delete this submission? This will also delete all images from Cloudinary.')) {
			return
		}
	...
	return (
		<div className={styles.dashboard}>
			...
			<div className={styles.stats}>
				...
				<div className={styles.statCard}>
					<h3>Total Images</h3>
					<p>{submissions.reduce((sum, s) => sum + (s.metadata?.totalImages || 0), 0)}</p>
				</div>
				<div className={styles.statCard}>
					<h3>Total Size</h3>
					<p>{formatBytes(submissions.reduce((sum, s) => sum + (s.metadata?.totalSize || 0), 0))}</p>
				</div>
			</div>
			...
			<div className={styles.submissionsList}>
				{submissions.length === 0 ? (
					<div className={styles.emptyState}>
						No submissions found{selectedUser && ` for user: ${selectedUser}`}
					</div>
				) : (
					submissions.map((submission) => (
						<div key={submission.submissionId} className={styles.submissionCard}>
							...
							<div className={styles.userInfo}>
								<strong>User:</strong> {submission.userIdentifier}
								{submission.userInfo.email && (
									<span className={styles.email}> ({submission.userInfo.email})</span>
								)}
							</div>

							<div className={styles.folderInfo}>
								<strong>Cloudinary Folder:</strong>
								<code className={styles.folderPath}>{submission.cloudinaryFolder}</code>
							</div>

							<div className={styles.imagesInfo}>
								<strong>Images:</strong> {submission.images.length}
								{submission.metadata?.totalSize && (
									<span className={styles.size}>
										{' '}({formatBytes(submission.metadata.totalSize)})
									</span>
								)}
							</div>
							...
							{submission.cloudinaryInfo && (
								<div className={styles.cloudinaryInfo}>
									<strong>Cloudinary Resources:</strong> {submission.cloudinaryInfo.resourceCount}
									{submission.cloudinaryInfo.resources.length > 0 && (
										<div className={styles.resourcesList}>
											{submission.cloudinaryInfo.resources.map((resource, index) => (
